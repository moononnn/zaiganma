...
class TestTranslateVarsToState(unittest.TestCase):
    def test_energy_tiers(self):
        # 五档边界：85 / 60 / 40 / 20
        self.assertIn("精神头很足", app.translate_vars_to_state({"energy": 100, "mood": 60, "affection": 0}, "玥儿"))
        self.assertIn("精神头很足", app.translate_vars_to_state({"energy": 85, "mood": 60, "affection": 0}, "玥儿"))
        self.assertIn("精力还不错", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 0}, "玥儿"))
        self.assertIn("稍微有点累", app.translate_vars_to_state({"energy": 40, "mood": 60, "affection": 0}, "玥儿"))
        self.assertIn("挺疲惫的", app.translate_vars_to_state({"energy": 20, "mood": 60, "affection": 0}, "玥儿"))
        self.assertIn("累得不行了", app.translate_vars_to_state({"energy": 0, "mood": 60, "affection": 0}, "玥儿"))

    def test_mood_tiers(self):
        self.assertIn("心情很好", app.translate_vars_to_state({"energy": 60, "mood": 100, "affection": 0}, "玥儿"))
        self.assertIn("心情还可以", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 0}, "玥儿"))
        self.assertIn("心情一般般", app.translate_vars_to_state({"energy": 60, "mood": 40, "affection": 0}, "玥儿"))
        self.assertIn("心情不太好", app.translate_vars_to_state({"energy": 60, "mood": 20, "affection": 0}, "玥儿"))
        self.assertIn("心情很差", app.translate_vars_to_state({"energy": 60, "mood": 0, "affection": 0}, "玥儿"))

    def test_same_direction_uses_dun(self):
        # energy 与 mood 同在高位 → "、"连接
        r = app.translate_vars_to_state({"energy": 90, "mood": 90, "affection": 0}, "玥儿")
        self.assertIn("、", r)
        self.assertNotIn("但", r)

    def test_opposite_direction_uses_dan(self):
        # energy 高 mood 低 → "但"连接
        r = app.translate_vars_to_state({"energy": 90, "mood": 10, "affection": 0}, "玥儿")
        self.assertIn("但", r)

    def test_affection_stages(self):
        self.assertIn("很亲密", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 90}, "玥儿"))
        self.assertIn("挺熟的", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 60}, "玥儿"))
        self.assertIn("相处有一阵", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 30}, "玥儿"))

    def test_low_affection_no_relation(self):
        # 好感度 < 21：不写关系描述，只返回状态行
        r = app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 10}, "玥儿")
        self.assertNotIn("玥儿", r)
        self.assertNotIn("亲密", r)
        self.assertNotIn("熟", r)

    def test_affection_boundaries(self):
        self.assertIn("很亲密", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 81}, "玥儿"))
        self.assertIn("挺熟的", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 51}, "玥儿"))
        self.assertIn("相处有一阵", app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 21}, "玥儿"))
        r = app.translate_vars_to_state({"energy": 60, "mood": 60, "affection": 20}, "玥儿")
        self.assertNotIn("玥儿", r)

    def test_defaults_when_missing(self):
        # 缺字段时用默认值 80/60/0，不抛异常
        r = app.translate_vars_to_state({}, "玥儿")
